...
const quotes = fileQuotes; // Replace with actual quotes data when available

const quotesPerPage = 25; // Initial number of quotes to display
...
export default function QuoteGrid() {
    ...
    const [staggeredQuotes, setStaggeredQuotes] = useState<typeof quotes>(quotes.slice(0, quotesPerPage));

    const setQuotesForPage = (page: number, quotesPerPage: number) => {
        const newQuotes = quotes.slice((page - 1) * quotesPerPage, page * quotesPerPage);

        setIsQuoteFadingOut(true);
        setTimeout(() => {
            setStaggeredQuotes(newQuotes);
    ...
    return (
        <div>
            <Paginator quotesPerPage={quotesPerPage} totalQuotes={quotes.length} onPageChange={(pageNum) => setQuotesForPage(pageNum, quotesPerPage)} />
